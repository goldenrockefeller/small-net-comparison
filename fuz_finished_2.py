...
fuz_std = (np.ones(30), np.ones(30), np.ones(30))
...
def eval_fuz(model, model_stats, x):
    ...
    # sqr_diff = (x-c) * (x-c)
    # v = jax.numpy.exp(sqr_diff / (smoothing * smoothing) )

    w = v/np.sum(v)

    return np.sum((b +0* m * (x-c)) * w)
...
def restat_fuz(model,model_mom, model_stats, x_s, rate):
    ...
    fuz_std = model_stats[0]
    smoothing = model_stats[1] * model_stats[1]
    ...
    smoothing = M/sum_weights
    smoothing_switch = model_stats[2]
    activate_smoothing = 1 - (
        (1-smoothing_switch)
        * (smoothing > (2 * old_smoothing))
        * (smoothing < (old_smoothing / 2))
    )
    smoothing_switch = activate_smoothing * (1.-1.*
        (smoothing < (1.5* old_smoothing))
        * (smoothing > (old_smoothing / 1.5))
    )

    smoothing = (1- smoothing_switch) * old_smoothing + smoothing_switch * smoothing
    smoothing = (1 - rate) *  old_smoothing + rate * smoothing


    model = (b, m, c)
    model_mom = (b_mom, m_mom, c_mom)
    model_stats = (np.sqrt(fuz_variance), np.sqrt(smoothing), smoothing_switch)
...
for i in range(8000):
    ...
    print(f"loss = {f(x,  (model_stats, x_s, y_s))}, epoch = {i}, {np.sum(model_stats[1][2])}")
