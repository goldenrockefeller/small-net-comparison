...
starting_model = rng.uniform(0, 1., size = (30))
a = rng.uniform(0, 1., size = (30))
def eval_model(model, x):

    b = np.hstack((a[1:], [1 + 1/29]))
    c = np.hstack(([- 1/29], (a[:-1])))
    l = np.sum((x >= a) * a * (x < b))
    h = np.sum((x < a) * a * (x >= c))
    ml =np.sum((x >= a) * model * (x < b))
    mh = np.sum((x < a) * model * (x >= c))

    return (ml * (h - x) + mh * (x - l)) / (h - l)
...
def f(vector):
    ...
    for x, y in zip(x_s, y_s):
        error_total += 0.5 * np.sum((eval_model(model, x) - y) * (eval_model(model, x) - y))
    return error_total
